...
3. **Set subscription price IDs secret** (JSON map `solo` / `studio` / `shop` / `charter` → Stripe Price id, plus **`smsExtra`** for extra texting lines). Use the committed example or root `stripe-price-ids.json`:
   ```bash
   firebase functions:secrets:set STRIPE_SUBSCRIPTION_PRICE_IDS --data-file stripe-price-ids.json
   firebase deploy --only functions
   ```
   Hand-typing price ids can mix up `I` vs `l` or break JSON — always use `--data-file`.

   **Extra SMS line ($12/mo):** `smsExtra` must be an **active** Stripe Price (e.g. `price_1U1w9XCee17fSOZIZfqPevuH`). Put retired prices in `smsExtraLegacy` so existing subscriptions still reconcile.
